[README.md](https://github.com/user-attachments/files/28048652/README.md)
# 无人机搜索算法仿真平台（UAV Search Algorithm Simulator）

用于二维环境下无人机目标搜索算法的仿真、可视化与对照实验评估。项目构建了统一的仿真平台，集成了栅格地图环境、概率观测模型与信念图系统，并实现了多种搜索策略算法（随机搜索、覆盖式搜索、前沿搜索、信息增益搜索及基于模板匹配的视觉引导搜索），用于系统比较不同算法在复杂环境中的性能表现。
